...
trait lib_trait {
    ...
    private function get_section_name($section, $current_index){
        ...
        if (get_string_manager()->string_exists("tw_course_format_{$build_name}", "local_fliplearning")) {
    ...
    protected function get_sessions($logs){
        $sessions = array();
        if(count($logs) == 0){
            return $sessions;
        }
    ...
    protected function get_active_days($logs){
        $days_count = 0;
        if(count($logs) == 0){
            return $days_count;
        }
